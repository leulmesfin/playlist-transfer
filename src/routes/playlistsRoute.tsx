...
const PlaylistsRoute = () => {
	...
	const toggleSelect = async(playlist: Playlist) => {
		const playlistFound = selectedPlaylists.some((p) => p.imageUrl === playlist.imageUrl)
		if (playlistFound) {
			// remove the playlist from playlists (don't mutate playlists, set playlists to new arr)
			setSelectedPlaylists(selectedPlaylists.filter((p) => p.imageUrl != playlist.imageUrl))
		} else {
			...
			{playlists.map((playlist: Playlist, index) => {
				...
				return (
					...
					<Card 
						...
						isSelected={selectedPlaylists.some((p) => p.imageUrl === playlist.imageUrl)}
...
const fetchPlaylistTracks = async (id: string) => {
	try {
		const resp = await fetch(`http://localhost:8080/api/get-playlist-tracks?playlistId=${id}`);
